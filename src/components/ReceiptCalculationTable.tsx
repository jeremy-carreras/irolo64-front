...
interface ReceiptCalculationTableProps {
  receipts: Receipt[];
}
...
export function ReceiptCalculationTable({ receipts }: ReceiptCalculationTableProps) {
  ...
  const calculateDepartmentReceipt = (dept: Department, receipt: Receipt): number => {
    ...
    try {
      ...
      return calculatedReceipt.totalPrice;
    } catch (err) {
      console.error(`Error calculating receipt for ${dept.code}:`, err);
      return 0;
    }
  };
  ...
  const calculateReceiptTotal = (receipt: Receipt): number => {
    return departments.reduce((total, dept) => {
      return total + calculateDepartmentReceipt(dept, receipt);
    }, 0);
  };

  const calculateCommonAreasAmount = (receipt: Receipt): number => {
    return receipt.totalCharge - calculateReceiptTotal(receipt);
  };
          ...
          <tbody>
            {departments.map((dept) => (
              <tr key={dept.id}>
                ...
                {receipts.map((receipt) => {
                  const amount = calculateDepartmentReceipt(dept, receipt);
                  return (
            ...
            <tr className="common-areas-row">
              ...
              {receipts.map((receipt) => {
                const amount = calculateCommonAreasAmount(receipt);
                return (
              ...
              {receipts.map((receipt) => {
                return (
                  <td key={`final-total-${receipt.id}`} className="amount" style={{ fontWeight: 'bold', backgroundColor: '#f0f0f0' }}>
                    {formatCurrency(receipt.totalCharge)}
                  </td>
                );
              })}
            </tr>
